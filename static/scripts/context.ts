...
export default {
  ...
  getCurrentSpan(table: number) {
    return realContext.tables[table][realContext.currentLine][
      realContext.currentLetter
    ];
  },
  addTable() {
  ...
  set responses(responses: (string | portugueseWord)[]) {
  ...
  get wordIsComplete() {
    return realContext.word.reduce(
  ...
  isValidResponse(response: string) {
    return realContext.responses.findIndex((res) =>
      typeof res === "string" ? res === response : res.transformed === response
    );
  },
  get isWin() {
...
interface Context {
  ...
  word: string[];
  responses: (string | portugueseWord)[];
  completes: number[];
}
